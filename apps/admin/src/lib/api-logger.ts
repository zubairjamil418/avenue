...
interface LoggerConfig {
  limit: number;
  level: LogLevel;
  /** When false (default), no logs are recorded at all */
  enabled: boolean;
}
...
const DEFAULT_CONFIG: LoggerConfig = {
  limit: 100,
  level: "error", // Only track errors by default
  enabled: false, // Tracking is OFF until admin explicitly enables
};
...
export const ApiLogger = {
  ...
  setConfig(config: Partial<LoggerConfig>) {
    ...
    window.dispatchEvent(new Event("api_logger_config_changed"));
  },
  ...
  addLog(log: Omit<ApiLogEntry, "_id" | "createdAt">) {
    ...
    // Guard 1: tracking must be enabled by admin
    if (!config.enabled) return;

    // Guard 2: level filter
