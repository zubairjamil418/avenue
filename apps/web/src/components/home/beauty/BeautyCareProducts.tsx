...
import BeautyCareProductsClient from "./BeautyCareProductsClient";

interface BeautyCareProductsProps {
  slug?: string;
  locale: string;
}

/**
 * Server component — fetches "beauty-care-products" products and renders the
 * Beauty Care Products section for the beauty (Home-2) page.
 */
const BeautyCareProducts = async ({
  slug = "beauty-care-products",
  locale,
}: BeautyCareProductsProps) => {
  ...
  try {
    ...
    console.error(`Error fetching ${slug} products:`, err);
  }

  if (products.length === 0) return null;

  return <BeautyCareProductsClient products={products} productType={productType} slug={slug} />;
};
